...
const prettyAuditAction=(action:string)=>{
  const map:Record<string,string>={
    ...
    delete_user:"Deleted radiologist",
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{backgroundColor:"rgba(0,0,0,.45)",backdropFilter:"blur(6px)"}}>
            <div className="anim-pop w-full max-w-md bg-white rounded-[32px] shadow-2xl overflow-hidden">
              <div className="px-8 pt-7 pb-6 text-white" style={{background:`linear-gradient(135deg,${DARK_GREEN},${ACCENT_GREEN})`}}>
                <div className="flex items-center justify-between"><h2 className="text-lg font-bold">Edit Patient</h2><button onClick={()=>setEditPatient(null)} className="w-9 h-9 rounded-full bg-white/20 hover:bg-white/30 flex items-center justify-center font-bold">X</button></div>
              </div>
            ...
            <div className="sticky top-4 z-20 mb-6 pt-4">
              <div className="w-full max-w-[1400px] mx-auto flex items-center justify-between gap-4 rounded-[22px] border border-slate-200 bg-white shadow-sm px-5 py-3.5">
                <div className="min-w-0">
                  ...
                  <p className="text-[12px] text-slate-500 mt-0.5">{myHospital?.name||"Hospital Admin"}</p>
                </div>
                ...
                <div className="grid grid-cols-12 gap-5">
                  ...
                  <Panel className="col-span-3 p-7 anim-in">
                    ...
                    <div className="space-y-3">
                      ...
                      <button onClick={()=>setTab("audit")} className="btn-s w-full py-3 rounded-2xl border text-sm font-bold" style={{background:"#EEF1F4",color:"#4A5A68",borderColor:"#D7DEE5"}}>Open Activity Log</button>
                    </div>
                  </Panel>
                </div>
                <div className="grid grid-cols-12 gap-5">
                    ...
                    <div className="space-y-4">
                      {visibleUsers.slice(0,4).map(u=>(
                        <div key={u.id} className="flex items-center justify-between">
                          <div className="flex items-center gap-3"><div className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center text-sm font-bold text-slate-700">{u.full_name.split(" ").map(w=>w[0]).join("").slice(0,2).toUpperCase()}</div><div><p className="text-[13px] font-bold text-slate-800">{u.full_name}</p><p className="text-[11px] text-slate-400">{u.hospital||u.role}</p></div></div>
                  ...
                  <Panel className="col-span-4 p-7 anim-in flex flex-col items-center">
                    ...
                    <div className="w-full space-y-3">
                      {[{label:"Registered Patients",value:visiblePatients.length}].map(item=>(
                  ...
                  <Panel className="col-span-3 p-7 anim-in">
                    <div className="flex flex-col h-full justify-between">
                      <div>
                        <h3 className="text-base font-bold text-slate-800 mb-3">Hospital Admin</h3>
                        <p className="text-[13px] text-slate-500 leading-relaxed">Manage radiologists, patient records and hospital activity from one place.</p>
                      </div>
                      <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4 mt-5">
                        <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Account Email</p>
                        <p className="text-sm font-bold text-slate-800 mt-2 break-all">{me?.email||"-"}</p>
                      </div>
                    </div>
                  </Panel>
                </div>
              ...
              <div className="space-y-4 max-w-[1200px] anim-in">
                ...
                <div className="space-y-2">
                  {visiblePatients.filter(p=>!search||p.name.toLowerCase().includes(search.toLowerCase())||(p.patient_ref_id&&p.patient_ref_id.includes(search))).map(p=>{
                    ...
                    return (
                      <div key={p.id} className="bg-white rounded-[24px] border overflow-hidden transition-all" style={{borderColor:"#E2E8F0",boxShadow:"0 2px 12px rgba(0,0,0,0.04)"}}>
                        ...
                        {isExp&&(
                          <div className="border-t border-slate-100 px-6 py-5 bg-slate-50/50">
                            ...
                            <div className="grid grid-cols-4 gap-3">
                              ...
                              <div className="rounded-2xl border border-slate-100 bg-white p-4">
                                <div className="text-[8px] font-bold uppercase text-slate-300">Age / Sex</div>
                                <div className="text-sm font-bold text-slate-800 mt-1">{p.age?`${p.age}y`:"-"} / {p.sex||"-"}</div>
                              </div>
                              <div className="rounded-2xl border border-slate-100 bg-white p-4">
                  ...
                  <Panel className="p-8 space-y-7">
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center gap-3">
                        <div className={`step-ring ${dxRadiologist?"done":"active"}`}>{dxRadiologist?"OK":"1"}</div>
                        <div>
                      ...
                      {dxRadiologist&&(()=>{const u=apiUsers.find(r=>r.id===dxRadiologist);return u?(<div className="flex items-center justify-between p-3 rounded-2xl" style={{background:"#F0FDF4",border:`1.5px solid ${BRAND}`}}>
                        ...
                        <button onClick={()=>{setDxRadiologist("");setDxRadSearch("");setSelectedPtId("");setPtSearch("");}} className="text-xs text-slate-400 hover:text-red-500 font-bold px-2">Clear</button>
                      </div>):null;})()}
                    </div>

                    <div className={`space-y-3 transition-opacity ${dxRadiologist?"opacity-100":"opacity-40 pointer-events-none"}`}>
                      <div className="flex items-center gap-3">
                        <div className={`step-ring ${selectedPtId?"done":dxRadiologist?"active":"idle"}`}>{selectedPtId?"OK":"2"}</div>
                        <div>
                      ...
                      {selectedPtId&&(()=>{const p=patients.find(pt=>pt.id===selectedPtId);return p?(<div className="flex items-center justify-between p-3 rounded-2xl" style={{background:"#EEF1F4",border:"1.5px solid #D7DEE5"}}>
                        ...
                        <button onClick={()=>{setSelectedPtId("");setPtSearch("");}} className="text-xs text-slate-400 hover:text-red-500 font-bold px-2">Clear</button>
                      </div>):null;})()}
                    </div>

                    <div className={`space-y-3 transition-opacity ${selectedPtId?"opacity-100":"opacity-40 pointer-events-none"}`}>
                      <div className="flex items-center gap-3">
                        <div className={`step-ring ${xFile?"done":selectedPtId?"active":"idle"}`}>{xFile?"OK":"3"}</div>
                        ...
                        {(["Normal","Pneumonia","Tuberculosis","Unknown"] as const).map(l=>{
                          ...
                          return(
                            <button key={l} onClick={()=>{setRtLabel(l);setRtFiles([]);}}
                              ...
                              <span>{l}</span>
                              {n>0&&<span className="block text-[10px] font-semibold mt-0.5 opacity-80">{n} staged {ready?"Ready":`(need ${RT_MIN-n} more)`}</span>}
                            ...
                            <div key={l} className="flex items-center justify-between">
                              ...
                              {n===0
                                ...
                                :<div className="flex items-center gap-2">
                                  ...
                                  <span className="text-[10px] font-bold" style={{color:ready?DARK_GREEN:SOFT_GREY}}>{n} {ready?"Ready":`/ ${RT_MIN}`}</span>
                                </div>}
                            </div>
                          );
                        })}
              ...
              <div className="space-y-5 max-w-[900px] anim-in">
                ...
                {modelInfo&&<div className="grid lg:grid-cols-2 gap-5">
                  <Panel className="p-7">
                    ...
                    {[{l:"Status",v:modelInfo.status},{l:"Architecture",v:modelInfo.architecture},{l:"Size",v:`${modelInfo.size_mb} MB`},{l:"Input Shape",v:modelInfo.input_shape?.join(" x ")},{l:"Last Modified",v:fmt(modelInfo.last_modified)}].map(r=>(
                  ...
                  {auditLogs.map(l=>{
                    ...
                    const detailText=
                      l.action==="delete_user"
                        ? `${l.detail?.target_name||"Radiologist"} removed by ${l.detail?.performed_by||actor}.`
                        : 
                    ...
                    return <TR key={l.id}>
                      ...
                      <TD>{l.detail?.patient_name||l.detail?.target_name||l.entity_id||"-"}</TD>
                      <TD mono>{fmt(l.timestamp)}</TD>
                      <TD>{detailText}</TD>
                    </TR>;
                  })}
                    ...
                    <div className="anim-pop w-full max-w-md bg-white rounded-[32px] shadow-2xl overflow-hidden">
                      <div className="px-8 pt-7 pb-6 text-white" style={{background:`linear-gradient(135deg,${DARK_GREEN},${ACCENT_GREEN})`}}>
                        <div className="flex items-center justify-between">
                          ...
                          <button onClick={()=>{setMyPwdModal(false);setMyPwd("");setMyPwdMsg("");}} className="w-9 h-9 rounded-full bg-white/20 hover:bg-white/30 flex items-center justify-center font-bold">X</button>
                        </div>
                      </div>
                      <div className="p-8 space-y-4">
