...
const IntroSequence: React.FC<IntroSequenceProps> = ({ onFinish }) => {
  ...
  useEffect(() => {
    ...
    const timer3 = setTimeout(() => setIsExiting(true), 3200);
  ...
  return (
    <div className={`fixed inset-0 z-[1000] bg-white flex items-center justify-center overflow-hidden transition-all duration-[1.5s] ease-in-out ${isExiting ? 'opacity-0 scale-105 pointer-events-none' : 'opacity-100 scale-100'}`}>
      ...
      <div className={`absolute inset-0 bg-emerald-900 transition-transform duration-[2s] ease-[cubic-bezier(0.85,0,0.15,1)] ${isExiting ? '-translate-y-full' : 'translate-y-0'}`} />

      {/* Words */}
      <div className="relative z-10 flex flex-col items-center">
        {words.map((word, i) => (
          <div key={i} className="h-16 md:h-24 overflow-hidden">
            <h1 className={`text-4xl md:text-8xl font-bold text-white transition-all duration-1000 ease-[cubic-bezier(0.23,1,0.32,1)] flex items-center tracking-tighter
              ${phase === i ? 'translate-y-0 opacity-100' : 'translate-y-full opacity-0'}`}>
              {word}
              {i === 2 && (
                <span className="ml-4 w-3 h-3 md:w-6 md:h-6 rounded-full bg-emerald-400 inline-block" />
              )}
            </h1>
          </div>
        ))}
